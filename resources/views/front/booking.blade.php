  ...
  <main class="bg-[#FAFAFA] max-w-[640px] mx-auto min-h-screen relative flex flex-col has-[#CTA-nav]:pb-[120px] has-[#Bottom-nav]:pb-[120px]">
      ...
      <div class="flex items-center w-full rounded-2xl border border-[#E9E8ED] p-4 gap-[10px] bg-white">
        ...
        <div class="flex flex-col">
          ...
          <div class="flex items-center gap-[2px]">
            <p class="text-sm leading-[21px] text-[#909DBF]">{{strip_tags($carStore->address)}}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="flex h-full flex-1 mt-5">
      <form method="POST" action="{{ route('front.booking.store', [$carStore->slug, $service->slug]) }}" class="w-full flex flex-col rounded-t-[30px] p-5 pt-[30px] gap-[26px] bg-white overflow-x-hidden mb-0 mt-auto" >
            ...
            <div class="swiper-wrapper w-full h-fit">
                @foreach (['09:00', '10:00', '11:00', '12:00', '13:00', '14:00', '15:00', '16:00'] as $time)
                    <label class="swiper-slide !w-fit group relative">
                        <div class="rounded-full !w-fit border border-[#E9E8ED] p-[12px_20px] font-semibold transition-all duration-300 hover:bg-[#5B86EF] hover:text-white bg-white group-has-[:checked]:bg-[#5B86EF] group-has-[:checked]:text-white group-has-[:disabled]:bg-[#EEEFF4] group-has-[:disabled]:text-[#AAADBF]">
                            {{ $time }}
                        </div>
                        <input type="radio" name="time_at" value="{{ $time }}" class="absolute top-1/2 left-1/2 -z-10" required>
                    </label>
                @endforeach
            </div>
        ...
        <div id="defaultOpen" class="w-full flex items-center justify-between bg-white">
          <div class="flex flex-col gap-[2px]">
            <p class="font-bold text-xl leading-[30px]">Rp {{ number_format($service->price, 2, ',', '.') }}</p>
